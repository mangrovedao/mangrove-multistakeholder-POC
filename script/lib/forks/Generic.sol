...
import {ToyENS} from "script/lib/ToyENS.sol";
...
contract GenericFork is Script {
  ...
  function readAddresses(string memory category) internal returns (Record[] memory) {
    ...
    bool readConfigPaths = vm.envOr("READ_ADDRESSES_PATHS", true);
    if (!readConfigPaths) {
      return envRecords;
    }

    Record[] memory ConfigRecords = new Record[](0);
    if (readConfigPaths) {
      ConfigRecords = readConfigAddresses(category);
    }

    Record[] memory allRecords = new Record[](envRecords.length + ConfigRecords.length);
    for (uint i = 0; i < envRecords.length; i++) {
      allRecords[i] = envRecords[i];
    }
    for (uint i = 0; i < ConfigRecords.length; i++) {
      allRecords[envRecords.length + i] = ConfigRecords[i];
    }
  ...
  function readEnvAddresses(string memory category) internal returns (Record[] memory) {
    ...
    string memory paths = vm.envOr("ADDRESSES_PATHS", defaultPath);
  ...
  function readConfigAddresses(string memory category) internal returns (Record[] memory) {
    string memory root = vm.projectRoot();
    string memory path = string.concat(root, "/config.json");
  ...
  function readAddressesFromJson(bytes memory encodedPaths, string memory category) internal returns (Record[] memory) {
    Config memory config = abi.decode(encodedPaths, (Config));
    Record[][] memory allRecords = new Record[][](config.addresses_paths.length);
    for (uint i = 0; i < config.addresses_paths.length; i++) {
      string memory pathFromRoot = addressesFile(config.addresses_paths[i], category, "");
